...
class ObjectInterceptorTest extends Specification {
    ...
    @Shared
    String RESOURCE_PATH = "${ROOT_PATH}/src/test/groovy/org/jalasoft/gradle/plugins/enforce/interceptor/resources/objects"
    @Shared
    String TRUNCATED_PATH = "${ROOT_PATH}/src/test/groovy/org/jalasoft/gradle/plugins/enforce/interceptor/resources/interceptor"
    ...
    def "Should execute the commands of page truncator"() {
        ...
        int totalSize = 0
        objectInterceptor.interceptorsToExecute = [org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_ACTION_OVERRIDES.id, org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_FIELD_SETS.id,
                                                 org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_FORMULAS.id, org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_WEB_LINKS.id]
        int total = 0        
        when:
    ...
    def "Should add new command as a first command"() {
        ...
        then:
        commands[0] == 'firstCmd'
        commands[1] == org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_FORMULAS.id
    }
    ...
    def "Should add new command as a last command"() {
        ...
        commands = objectInterceptor.interceptors.keySet().toArray()
        then:
        commands[commands.length - 2] == org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_ACTION_OVERRIDES.id
    ...
    def "Should add new command in the third index"() {
        ...
        def commands = objectInterceptor.interceptors.keySet().toArray()
        then:
        commands[3] == org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_FIELD_SETS.id
        commands[2] == 'middleCmd'
        commands[1] == org.jalasoft.gradle.plugins.enforce.interceptor.Interceptor.TRUNCATE_WEB_LINKS.id
    }
